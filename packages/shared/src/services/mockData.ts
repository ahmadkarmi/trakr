...
function initializeNotificationsForSubmittedAudits() {
  console.log('🔍 Checking for SUBMITTED audits to create notifications...')
  ...
  mockAudits.forEach(audit => {
    ...
    if (!audit.submittedAt) {
      console.log(`⚠️ Audit ${audit.id} is SUBMITTED but has no submittedAt date`)
      return
    }
    ...
    if (existingNotification) {
      console.log(`ℹ️ Notification already exists for audit ${audit.id}`)
    ...
    if (assignments.length === 0) {
      console.log(`⚠️ No managers assigned to branch ${audit.branchId}`)
    ...
    assignments.forEach(assignment => {
      ...
      console.log(`✅ Created notification for manager ${assignment.managerId} (${auditor?.email}) for audit ${audit.id}`)
    })
  })
  
  console.log(`📊 Total notifications now: ${mockNotifications.length}`)
}
...
export const mockApi = {
  ...
  getNotifications: async (userId: string): Promise<Notification[]> => {
    ...
    console.log(`📊 Total mockNotifications before init: ${mockNotifications.length}`)
